...
public record TopicDTO(
    ...
    public static TopicDTO fromEntity(TopicEntity topic, UserDTO user, CourseDTO course, Long repliesCount) {
        return new TopicDTO(
                topic.getId(),
                topic.getTitle(),
                topic.getDescription(),
                course.name(), // topic.getCourse().getName(),
                course.category(), // topic.getCourse().getCategory(),
                user.username(), // topic.getUser().getUsername(),
                repliesCount, // topic.getResponses().size(),
                topic.getStatus(),
                topic.getCreatedAt(),
                topic.getUpdatedAt()
        );
