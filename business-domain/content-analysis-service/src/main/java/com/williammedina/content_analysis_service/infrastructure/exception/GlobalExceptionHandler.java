package com.williammedina.content_analysis_service.infrastructure.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleNonTransientAiException(NonTransientAiException ex, HttpServletRequest request) {
        log.error("Error sending AI prompt: {}", ex.getMessage());
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                buildError(HttpStatus.INTERNAL_SERVER_ERROR, "Error enviando prompt a la IA",  request.getRequestURI(), null)
        );
    }
    ...
    public ResponseEntity<ApiErrorResponse> handleResourceAccessException(ResourceAccessException ex, HttpServletRequest request) {
        log.error("Error accessing resource: {}", ex.getMessage());
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                buildError(HttpStatus.INTERNAL_SERVER_ERROR, "Error accediendo al recurso externo",  request.getRequestURI(), null)
        );
    }
